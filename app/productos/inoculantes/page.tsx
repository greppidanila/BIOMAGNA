...
export default function InoculantesPage() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [expandedProduct, setExpandedProduct] = useState<string | null>(null);
  const [downloadSuccess, setDownloadSuccess] = useState<string | null>(null);

  const categoryName = "Inoculantes";
  const categoryDef = "Los Inoculantes biológicos introducen bacterias altamente infectivas y efectivas para fijar Nitrógeno simbiótico directamente en los nódulos radiculares principales. Incluyen protectores bio-osmóticos avanzados que duplican la viabilidad de la bacteria residual sobre el tegumento seminal, logrando una pre-siembra segura de hasta 25 días.";

  const products = [
    {
      id: "protege-ts",
      name: "PROTEGE TS",
      logo: "/imgi_2_PROTEGE-TS-1.png",
      oneLineDesc: "Protector de inoculado osmótico de biopolímeros biodegradables diseñado para pre-tratamiento prolongado de semilla (pre-siembra).",
      fullDesc: "Protege TS es un polímero protector biológico de última generación que actúa aislando mecánicamente a las bacterias benéficas Bradyrhizobium de la desecación climática, del choque químico con fungicidas sintéticos curasemillas y del estrés osmótico natural en suelos secos.",
      specs: [
        { key: "Propiedad Principal", val: "Polímero protector bacteriano osmótico biodegradable" },
        { key: "Viabilidad Residual", val: "Mantiene bacterias viables en semilla hasta por 25 días" },
        { key: "Compatibilidad", val: "Homologado para tratamientos mixtos con curasemillas y nitrógeno" },
        { key: "Dosis Recomendada", val: "50 mL por cada 100 kg de semilla (combinado con Totalnitro)" }
      ],
      advantages: ["Extiende el período disponible para siembra anticipada", "Previene la muerte celular bacteriana ante altas temperaturas en lote", "Forma una película uniforme que optimiza la fluidez en sembradoras"]
    },
    {
      id: "totalnitro",
      name: "TOTALNITRO",
      logo: "/imgi_3_total-nitro-1-2048x366-1.png",
      oneLineDesc: "Inoculante biológico ultra-concentrado con cepas seleccionadas de bacterias nodulantes Bradyrhizobium japonicum de máxima nodulación.",
      fullDesc: "Totalnitro de Biomagna es una fórmula ultra-concentrada de Bradyrhizobium japonicum en soporte líquido purificado. Diseñado con cepas altamente competitivas que colonizan rápidamente los pelos radiculares, induciendo la formación de grandes nódulos en la raíz principal para una fijación biológica de nitrógeno (FBN) ininterrumpida.",
      specs: [
        { key: "Microorganismo", val: "Bradyrhizobium japonicum (cepas seleccionadas de alta infectividad)" },
        { key: "Concentración", val: "Mínimo 3 x 10^9 células viables/mL al envasarse" },
        { key: "Cultivo", val: "Exclusivo Leguminosas (principalmente soja y alfalfa)" },
        { key: "Dosis Recomendada", val: "100 mL cada 100 kg de semilla de soja" }
      ],
      advantages: ["Asegura un suministro óptimo de nitrógeno natural sin costo de fertilización química", "Cepa sumamente adaptada a suelos con antecedentes desfavorables", "Aumento probado de proteína total y peso del grano cosechado"]
    }
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FAF8F5] text-[#2C3322] font-sans relative" id="inoculantes-root">
      ...
      {/* HERO BANNER SECTION */}
      <section className="bg-gradient-to-r from-[#173F1D] to-[#2E5E2E] text-white py-16 px-4 relative overflow-hidden" id="category-hero-banner">
        <div className="absolute inset-0 z-0 opacity-25 select-none pointer-events-none">
          <Image 
            src="https://images.unsplash.com/photo-1574375927938-d5a98e8edd86?q=80&w=1200"
            alt="Fondo cultivo leguminosa nodulos inoculante"
            fill
        ...
        <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-8 space-y-4">
            <div className="flex items-center gap-2 text-[#E8D385] text-xs font-mono font-bold tracking-widest uppercase">
              <Layers className="w-4 h-4 text-[#E8D385]" />
              <span>Fijación Simbiótica</span>
            </div>
          ...
          <div className="md:col-span-4 hidden md:flex justify-end">
            <div className="relative w-72 h-44 rounded-2xl overflow-hidden border border-white/20 shadow-2xl skew-y-1 hover:skew-y-0 transition-transform duration-500 bg-[#173F1D]">
              <Image 
                src="https://images.unsplash.com/photo-1574375927938-d5a98e8edd86?q=80&w=600"
                alt="Tecno Inoculantes Bio"
                fill
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-3.5 pt-8">
                <span className="text-[10px] font-mono text-[#E8D385] tracking-widest uppercase font-bold block mb-0.5">Simbiosis Eficiente</span>
                <span className="text-xs text-white/90 font-bold block">Tratamiento de Semilla</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-16 flex-grow">
        <h2 className="text-xs uppercase font-mono font-black text-[#4C533C]/60 tracking-widest border-b border-[#EAE5DB] pb-3 mb-10">
          Inoculantes Líderes para Tratamiento de Semilla
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8" id="inoculantes-products-grid">
                  ...
                  {expandedProduct === p.id && (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {/* Specs */}
                        <div>
                          <p className="font-bold text-[#1E2315] mb-2 flex items-center gap-1"><Info className="w-3.5 h-3.5" /> Ficha de Registro</p>
                        ...
                        {/* Advantages */}
                        <div>
                          <p className="font-bold text-[#1E2315] mb-2 flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-emerald-600" /> Beneficios Clave</p>
