...
export default function BiodefensivosPage() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [showSpecs, setShowSpecs] = useState(false);
  const [downloadSuccess, setDownloadSuccess] = useState(false);

  // Category values
  const categoryName = "Biodefensivos";
  const categoryDef = "Los Biodefensivos integran microorganismos vivos que combaten activamente plagas y fitopatógenos específicos. Son la columna vertebral del Manejo Integrado de Plagas (MIP), ya que controlan poblaciones de insectos nocivos sin generar resistencias fisiológicas ni dejar residuos químicos.";

  const product = {
    name: "BIOKATO",
    logo: "/imgi_2_Biokato.png",
    oneLineDesc: "Consorcio microbiológico de máxima eficacia selectiva diseñado para el control biológico sustentable de Chicharrita (Dalbulus maidis) y plagas complejas.",
    description: "Biokato es una formulación biológica líder que actúa mediante esporas fúngicas encapsuladas de Beauveria bassiana. Al tomar contacto con la cutícula del insecto, produce enzimas líticas que degradan el tegumento para su colonización biológica, deteniendo el daño al cultivo de maíz a las pocas horas de la aplicación.",
    specs: [
      { label: "Microorganismo Activo", val: "Beauveria bassiana cepa seleccionada Biomagna BM-02" },
      { label: "Titulación Concentración", val: "Mínimo 1 x 10^10 Conidios Viables por mililitro (alta carga biológica)" },
      { label: "Objetivo Principal", val: "Chicharrita de Maíz (Dalbulus maidis), Trips, Moscas Blancas de difícil control" },
      { label: "Dosis Recomendada", val: "1.0 a 1.5 Litros por Hectárea (según monitoreo poblacional)" },
      { label: "Presentación", val: "Bidones de 5 y 10 Litros purificados, listos para mezcla de pulverización" },
      { label: "Clasificación SENASA", val: "Insumo Biológico Registrado (Establecimiento Certificado N° 41.660)" }
    ],
    dosages: [
      { step: "1. Monitoreo", detail: "Iniciar aplicaciones tempranas ante la presencia de los primeros adultos colonizadores en V2-V4." },
      { step: "2. Preparación", detail: "Diluir la dosis de Biokato en agua limpia con corrector de pH (óptimo 5.5 a 6.5)." },
      { step: "3. Aplicación", detail: "Preferentemente durante el crepúsculo o días nublados para proteger la viabilidad fúngica inicial." }
    ]
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FAF8F5] text-[#2C3322] font-sans relative" id="biodefensivos-root">
      ...
      {/* HERO BANNER SECTION */}
      <section className="bg-gradient-to-r from-[#1D2214] to-[#343F24] text-white py-16 px-4 relative overflow-hidden" id="category-hero-banner">
        {/* Subtle decorative plant overlay background */}
        <div className="absolute inset-0 z-0 opacity-25 select-none pointer-events-none">
          <Image 
            src="https://images.unsplash.com/photo-1599599810769-bcfe5a160d32?q=80&w=1200"
            alt="Fondo biodefensivos biológico"
            fill
        ...
        <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-8 space-y-4">
            <div className="flex items-center gap-2 text-[#E8D385] text-xs font-mono font-bold tracking-widest uppercase">
              <ShieldCheck className="w-4 h-4 text-[#E8D385]" />
              <span>Protección Biológica</span>
            </div>
          ...
          <div className="md:col-span-4 hidden md:flex justify-end">
            <div className="relative w-72 h-44 rounded-2xl overflow-hidden border border-white/20 shadow-2xl skew-y-1 hover:skew-y-0 transition-transform duration-500 bg-[#1D2214]">
              <Image 
                src="https://images.unsplash.com/photo-1599599810769-bcfe5a160d32?q=80&w=600"
                alt="Tecno Biodefensivos Avanzados"
                fill
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-3.5 pt-8">
                <span className="text-[10px] font-mono text-[#E8D385] tracking-widest uppercase font-bold block mb-0.5">Control Premium</span>
                <span className="text-xs text-white/90 font-bold block">Biokato Activo</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-16 flex-grow">
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          <h2 className="text-xs uppercase font-mono font-black text-[#4C533C]/60 tracking-widest border-b border-[#EAE5DB] pb-3" id="main-product-title">
            Ficha del Producto Especializado
          </h2>

          {/* Core product card requested */}
          <motion.div 
            initial={{ opacity: 0, scale: 0.98 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.4 }}
            className="bg-[#FAF8F5] border-4 border-[#EAE5DB]/40 rounded-3xl p-6 sm:p-10 shadow-sm relative overflow-hidden" 
            id="product-biokato-card"
          >
            {/* Soft background decor */}
            <div className="absolute top-0 right-0 w-44 h-44 bg-gradient-to-bl from-[#4C533C]/5 to-transparent rounded-bl-full pointer-events-none" />
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 border-b border-[#EAE5DB] pb-8">
                ...
                <div className="bg-[#4C533C]/10 text-[#4C533C] px-3 py-1 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider">
                  MIP Solución Homologada
                </div>
              </div>

              {/* One-line description and detailed text */}
              ...
              {/* Action Buttons: "Ver más" (toggle specs) and "Ficha Técnica" */}
              ...
              {/* Interactive Slide down Content for "Ver más" */}
              <AnimatePresence>
                {showSpecs && (
                  <motion.div
                    initial={{ opacity: 0, height: 0 }}
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-4">
                        <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-[#1E2315]">Especificaciones Técnicas:</h4>
                      ...
                      <div className="space-y-4">
                        <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-[#1E2315]">Método de Aplicación Sugerido:</h4>
              ...
              {/* Success Notification Alert for Technical Sheet Download */}
              <AnimatePresence>
                {downloadSuccess && (
                  <motion.div
                  ...
                  >
                    ...
                    <div>
                      <p className="font-bold">Ficha Técnica Iniciada Correctamente</p>
                      <p className="font-light mt-0.5 opacity-90">Se está preparando el archivo oficial homologado por SENASA para el Lote Biomagna-Biokato-FichaTecnica.pdf (Simulación de descarga completada).</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
